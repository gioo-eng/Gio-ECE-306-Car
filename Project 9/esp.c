...
void ESP_ProcessStartup(const char *frame)
{
    ...
    {
        if (   (strstr(frame, ESP_WIFI_SSID) != 0)
            || (strstr(frame, "WIFI GOT IP") != 0) )
        {
...
void execute_iot_command(const ESPCommandEvent *evt) {
    ...
    switch (evt->direction) {
        case ESP_DIR_FORWARD:
            ...
            // time_units is seconds. Multiply by 1000 for milliseconds.
        ...
        case ESP_DIR_RIGHT:
            // Spin CW: Left goes forward, Right goes reverse
           RIGHT_FORWARD_SPEED = t_R;
           LEFT_REVERSE_SPEED  = t_L;
            // time_units is degrees. You will need to tune the '10' multiplier 
            // to match how fast your car actually spins.
            ms_delay(evt->time_units * 8); 
            break;

        case ESP_DIR_LEFT:
            // Spin CCW: Right goes forward, Left goes reverse
